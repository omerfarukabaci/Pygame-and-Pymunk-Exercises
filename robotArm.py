...
while running:
    ...
    #---Robot Arm Grabbing Part Starts---#

    circle.body.velocity = (0,0)
    
    if int(block.body.position[1])+58 == circle.body.position[1] and (circle.body.position[0] > block.body.position[0]- 58 and circle.body.position[0] < block.body.position[0] + 58) and not waitStatus or stickState:
        stickPos = (realPos[0],realPos[1]-50)
        block.body.position = stickPos
        block.body.velocity = (0,0)
        stickState = True
        floorState = False
    
    if int(block.body.position[1]) <= 115 and not floorState:
        floorPos = (realPos[0],realPos[1]-50)
        block.body.position = floorPos
        floorState = True
        stickState = False
        waitStatus = True

    if circle.body.position[1] > 130:
        waitStatus = False
